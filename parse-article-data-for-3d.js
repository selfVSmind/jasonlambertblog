...
const renderer = {
  heading(text, level) {
    return `{ "type": "header", "data": { "headerLevel": "${level}", "headerText": "${he.decode(text)}" } },`;
  },
  ...
  paragraph(text) {
    try {
      ...
      if(text === "") return "";
      return `{ "type": "paragraph", "data": { "text": "${he.decode(text)}" } },`;
    }
    // json parsing worked
    if(parsedJson['type']) {
...
getArticleData = (filePath, articleDir) => {
  ...
  interpretedContent = interpretedContent.slice(0, interpretedContent.lastIndexOf(','));
